...
Dim SumNR As Double
    ...
    End If
    s = "select * from t_STTPembayaran" & pTipe & " where TanggalGiro between " & cD(mParams(0)) & " and " & cD(mParams(1)) & MyFilter & " and CaraBayar='GIRO' order by NoSTT"
    Set rs1 = Nothing
    rs1.Open s, CN, adOpenKeyset, adLockOptimistic
    s = "select t_STT~.NoSTT, NoFaktur, t_STTPelunasan~.Nilai from t_STTPelunasan~ left join t_STT~ on t_STTPelunasan~.NoSTT=t_STT~.NoSTT where TanggalGiro between " & cD(mParams(0)) & " and " & cD(mParams(1)) & MyFilter2 & " and GiroNum<>0 order by t_STT~.NoSTT"
    Set rs2 = Nothing
    s = Replace(s, "~", pTipe)
    rs2.Open s, CN, adOpenKeyset, adLockOptimistic
    s = "select distinct t_STTPotongan~.NoSTT, right(t_STTPotongan~.NoSTT,2),left(t_STTPotongan~.NoSTT,4), t_STTPotongan~.NoBukti, t_STTPotongan~.Nilai from t_STTPotongan~ left join t_STTPembayaran~ on t_STTPembayaran~.NoSTT=t_STTPotongan~.NoSTT where TanggalGiro between " & cD(mParams(0)) & " and " & cD(mParams(1)) & MyFilter3 & " and t_STTPotongan~.Nilai>0 and CaraBayar='GIRO' order by t_STTPotongan~.NoSTT"
    Set rs3 = Nothing
    s = Replace(s, "~", pTipe)
