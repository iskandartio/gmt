...
Begin VB.Form FormMasterKeterangan 
   BackColor       =   &H00FFC0C0&   Caption         =   "KETERANGAN"
   ClientHeight    =   5460
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   9615
   ...
   Begin TrueOleDBGrid80.TDBGrid TDBGrid1 
      ...
      Splits(0)._ColumnProps(5)=   "Column(1).Width=741"
      Splits(0)._ColumnProps(6)=   "Column(1).DividerColor=0"
      Splits(0)._ColumnProps(7)=   "Column(1)._WidthInPix=661"
      Splits(0)._ColumnProps(8)=   "Column(1).Order=2"
      Splits(0)._ColumnProps(9)=   "Column(2).Width=1058"
      Splits(0)._ColumnProps(10)=   "Column(2).DividerColor=0"
      Splits(0)._ColumnProps(11)=   "Column(2)._WidthInPix=979"
      Splits(0)._ColumnProps(12)=   "Column(2).Order=3"
      Splits(0)._ColumnProps(13)=   "Column(3).Width=4339"
      Splits(0)._ColumnProps(14)=   "Column(3).DividerColor=0"
      Splits(0)._ColumnProps(15)=   "Column(3)._WidthInPix=4260"
      Splits(0)._ColumnProps(16)=   "Column(3).Order=4"
      Splits(0)._ColumnProps(17)=   "Column(3)._MinWidth=149"
      Splits.Count    =   1
   ...
   End
End
Attribute VB_Name = "FormMasterKeterangan"
...
Private Sub fDelete_Click()
    If col1(icId).Value = "" Then Exit Sub
    a = "delete from m_Keterangan where IdKet=" & col1(icId).Value
    ExecMe a
    DoQuery
End Sub


Private Sub fKet_KeyPress(KeyAscii As Integer)
    col1(iUpdated).Value = 1
End Sub

Private Sub fKet_Validate(Cancel As Boolean)
    col1(iKet).Value = fKet
End Sub
...
Private Sub DoQuery()
    a = "select 0, IdDet, IdKet, Keterangan from m_Keterangan order by IdKet"
    query a
...
Private Sub Form_Load()
    ...
    DoQuery
    TDBGridSetVisible TDBGrid1, iUpdated & "@" & icId
...
On Error GoTo err
    ...
    For i = 0 To x.UpperBound(1)
        If x(i, iUpdated) = 1 Then
            If x(i, icId) <> "" Then
                a = "update m_Keterangan set IdKet=" & x(i, iId) & _
                    ", Keterangan='" & esc(x(i, iKet)) & _
                    "' where IdDet=" & x(i, icId)
                ExecMe a
            Else
                a = "insert into m_Keterangan(IdKet, Keterangan) values(" & _
                    x(i, iId) & _
                    ",'" & esc(x(i, iKet)) & "')"
                ExecMe a
            End If
