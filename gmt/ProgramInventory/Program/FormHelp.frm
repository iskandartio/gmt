...
Dim f As Form
Dim mRetCol As Integer
Dim fFind As String
Dim mUnload As Boolean

Sub LoadMe(ByVal tSQL As String, ByVal tHeaders As String, ByVal tWidths As String, ByVal tf As Form, Optional ByVal tLeft As Single = 0, Optional ByVal tTop As Single = 0, Optional ByVal tRetCol As Integer = 0)
On Error GoTo err
    query tSQL
With TDBGrid1
...
Dim w As Double
    ...
    b = Split(tWidths, "@")
    For i = 0 To UBound(a)
        .Columns.Add(i).Visible = IIf(Left(a(i), 1) = "*", False, True)
        .Columns(i).Caption = a(i)
        .Columns(i).Width = b(i)
        w = w + b(i)
    Next
    x.ReDim 0, 0, 0, .Columns.Count - 1
    x.DeleteRows 0
...
On Error Resume Next
    If KeyCode = 13 Then
        a = TDBGrid1.Columns(mRetCol).Value
        f.ActiveControl.Text = a
        f.FormHelpKeyDown a
        mUnload = True
        fCari.SetFocus
    End If
...
Private Sub TDBGrid1_KeyPress(KeyAscii As Integer)
    If (KeyAscii > 47 And KeyAscii < 123) Or KeyAscii = 32 Then
