...
Private Function MyFilter() As String
    If cD(fAwal) <> "A" Then MyFilter = MyFilter & " and TanggalOK>=" & cD(fAwal)
    If cD(fAkhir) <> "A" Then MyFilter = MyFilter & " and TanggalOK<=" & cD(fAkhir)
End Function

Private Sub DoQuery()
    a = "select distinct NoBukti, TanggalOK, DeptAsal, StatusPindahStock from t_Out where Dept='" & esc(fDept) & "' and FromNPB=0" & MyFilter & " order by TanggalOK Desc, NoBukti"
    query a
...
On Error Resume Next
    ...
    a = "select t_Out.IdStock, NamaBarang, QTY, Satuan, t_Out.Keterangan from t_Out left join m_StockBeli on t_Out.IdStock=m_stockBeli.IdStock where NoBukti='" & esc(col1("No Bukti").Value) & "'"
    query a
...
On Error GoTo err
    BeginTransaction
    a = "select IdStock, Dept, QTY from t_In where NoBukti='" & esc(col1("No Bukti").Value) & "'"
    query a
    While Not RS.EOF
        a = "update m_StockProses set JumlahTerpakai=JumlahTerpakai-" & cNum(RS.Fields("QTY").Value) & ",Jumlah=Jumlah-" & cNum(RS.Fields("QTY").Value) & " where IdStock=" & RS.Fields("IdStock").Value & " and Dept='" & esc(RS.Fields("Dept").Value) & "'"
        If ExecMe(a) = 0 Then GoTo err
        RS.MoveNext
    Wend
    a = "update t_In set StatusPindahStock=1 where NoBukti='" & esc(col1("No Bukti").Value) & "'"
    ExecMe a
    a = "update t_Out set StatusPindahStock=1 where NoBukti='" & esc(col1("No Bukti").Value) & "'"
    ExecMe a
    For i = 0 To y.UpperBound(1)
        a = "update m_StockProses set Jumlah=Jumlah+" & cNum(y(i, col2("QTY").ColIndex)) & " where IdStock=" & y(i, col2("IdStock").ColIndex) & " and Dept='" & esc(fDept) & "'"
        If ExecMe(a) = 0 Then GoTo err
    Next
    CommitTransaction
    MsgBox "SUKSES"
...
On Error GoTo err
    ...
    query a
    While Not RS.EOF
        a = "update m_StockProses set Jumlah=Jumlah+" & cNum(RS.Fields("QTY").Value) & ", JumlahTerpakai=JumlahTerpakai+" & cNum(RS.Fields("QTY").Value) & " where IdStock=" & RS.Fields("IdStock").Value & " and Dept='" & esc(RS.Fields("Dept").Value) & "'"
        ExecMe a
        RS.MoveNext
    Wend
    a = "update t_Out set StatusPindahStock=0 where NoBukti='" & esc(col1("No Bukti").Value) & "'"
    ExecMe a
    a = "update t_In set StatusPindahStock=0 where NoBukti='" & esc(col1("No Bukti").Value) & "'"
    ExecMe a
    For i = 0 To y.UpperBound(1)
        a = "update m_StockProses set Jumlah=Jumlah-" & cNum(y(i, col2("QTY").ColIndex)) & " where IdStock=" & y(i, col2("IdStock").ColIndex) & " and Dept='" & esc(fDept) & "'"
        If ExecMe(a) = 0 Then GoTo err
...
Private Sub Form_Load()
    ...
    fDept.ZOrder 0
    fAkhir = pServerDate
    fAwal = add_tanggal(pServerDate, -30)
