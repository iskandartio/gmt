...
"' and c.NoWarna='" & tNoWarna & "' and b.Tanggal > " & tTahun & "0000"
    ...
    Public Function qDataPrintedPacking(ByVal tDate As Date) As DataTable
        Dim s As String
        s = "select 0 as [Check], Shift, Tanggal, NoBukti, Lot, Jenis, KodeBarang, Warna, NoWarna, Tube, Grade, SatBesar, SatKecil, m_Stock~.IdStock, n1,n2, Keterangan, PrintedCode, IDTrans from t_InputStock~ left join m_Stock~ on t_InputStock~.IdStock=m_Stock~.IdStock where Tanggal=" & convertToInteger(tDate) & " and Status=1 order by NoBukti, m_stock~.IdKodeBarang, KodeBarang"
        Return a.doQuery(s)
    End Function
