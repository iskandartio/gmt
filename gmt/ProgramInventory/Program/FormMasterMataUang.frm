...
Begin VB.Form FormMasterMataUang 
   BackColor       =   &H00FFC0C0&   Caption         =   "MATA UANG"
   ClientHeight    =   4665
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7215
   ...
   Begin TrueOleDBGrid80.TDBGrid TDBGrid1 
      ...
      Columns(3).Caption=   "Mata Uang"
      Columns(3).DataField=   ""
      Columns(3)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
      Columns(4)._VlistStyle=   0
      Columns(4)._MaxComboItems=   5
      Columns(4).Caption=   "Negara"
      Columns(4).DataField=   ""
   ...
   Begin VB.Label Label1 
      BackColor       =   &H00C0FFFF&
      Caption         =   "Click Update untuk Update Mata Uang"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FormMasterMataUang"
...
Private Sub DoQuery()
    a = "select 0, Tipe, Kode, Nama, Negara from m_matauang order by Kode"
    query a
...
On Error GoTo err
    ...
    For i = 0 To x.UpperBound(1)
        If x(i, col1("updated").ColIndex) = "1" Then
            a = "update m_matauang set Kode='" & x(i, col1("Kode").ColIndex) & _
                "', Nama='" & x(i, col1("Mata Uang").ColIndex) & _
                "', Negara='" & x(i, col1("Negara").ColIndex) & _
                "' where Tipe='" & x(i, col1("Tipe").ColIndex) & "'"
            ExecMe a
        End If
    Next
    CommitTransaction
