...
Private Sub Form_Load()
    ...
    If ExecMe(s) = -1 Then
        s = "create table m_Karyawan(NIK long PRIMARY KEY, Nama text(255), Departemen text(50), Gaji Double)"
        ExecMe s
    End If
...
Private Sub fKaryawan_Click()
    ...
    Next
    a = "select 0 as updated, NIK as pKey, m_Karyawan.* from m_Karyawan where 1=1 " & MyFilter & " order by NIK"
    query a
...
Dim rec As New ADODB.Recordset
    ...
    For i = 0 To x.UpperBound(1)
        If CLng(x(i, 0)) = 1 Then
            ...
            If mTitle = "KARYAWAN" Then
                s = "select top 1 * from m_Karyawan where NIK=" & c
                Set rec = Nothing
                rec.Open s, CN, adOpenKeyset, adLockOptimistic
                If Not rec.EOF Then
                    For j = 0 To rec.Fields.Count - 1
                        rec.Fields(j).Value = x(i, j + 2)
                    Next
                    rec.Update
                End If
            ElseIf mTitle = "DEPARTEMEN" Then
