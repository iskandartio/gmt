...
Begin VB.Form FormHistoryPenjualan 
   BackColor       =   &H00FFC0C0&
   Caption         =   "HISTORIS PENJUALAN 3 BULAN TERAKHIR"
   ClientHeight    =   6120
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11025
   ...
   Begin TrueOleDBGrid80.TDBGrid TDBGrid1 
      ...
      Columns(1).Caption=   "No SC"
      Columns(1).DataField=   ""
      Columns(1)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
      Columns(2)._VlistStyle=   0
      Columns(2)._MaxComboItems=   5
      Columns(2).Caption=   "Nama Customer"
      Columns(2).DataField=   ""
   ...
   End
End
Attribute VB_Name = "FormHistoryPenjualan"
...
Sub LoadMe(ByVal tCustomer As String, ByVal tNamaBarang As String, ByVal tf As Form)
    ...
    fTanggalAwal = add_tanggal(pServerDate, -120)
...
Private Sub DoQuery()
    a = "SELECT t_SPP" & pTipe & ".NoSJ, NoSC, Nama, Jenis+' '+KodeBarang+' '+Warna+' '+NoWarna+' '+Tube+' GRADE '+Grade, t_SPPDetail" & pTipe & ".JumlahKG, SatKecil, Harga, t_SPPDetail" & pTipe & ".IdStock FROM (m_Customer INNER JOIN (t_SPPDetailPE INNER JOIN t_SPPPE ON t_SPPDetail" & pTipe & ".NoSPP = t_SPP" & pTipe & ".NoSPP) ON m_Customer.Kode = t_SPP" & pTipe & ".Kode) INNER JOIN m_Stock" & pTipe & " ON t_SPPDetail" & pTipe & ".IdStock = m_Stock" & pTipe & ".IdStock " & _
        " where Status>1 and Nama like '" & esc(fCustomer) & _
        "%' and Jenis+' '+KodeBarang+' '+Warna+' '+NoWarna+' '+Tube+' GRADE '+Grade like '%" & esc(fNamaBarang) & _
        "%' and TanggalSJ>=" & cD(fTanggalAwal) & _
        " and TanggalSJ<=" & cD(fTanggalAkhir)
    query a
