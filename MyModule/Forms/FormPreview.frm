...
Dim mPageSign() As String
Dim mTotalPage As Integer
Dim mSetPageHeight As Boolean
Dim mLandscape As Byte
Dim mHide As String
Dim f As Form
Private Const WM_MOUSEWHEEL = &H20A
Private Const WM_KEYDOWN As Long = &H100

Sub SetLandscape(ByVal tLanscape As Byte)
    mLandscape = tLanscape
End Sub
...
Sub LoadMe(tf As Form, ByVal tReportForm As String, Optional ByVal tParams As String, Optional ByVal tSQL As String, Optional ByVal tSQLAfterPrint As String, Optional ByVal tPageHeight As Single = 0)
    ...
    s = GetSetting(App.EXEName, "Printer", "Name", fPrinter)
    For i = 0 To Printers.Count - 1
        If Printers(i).DeviceName = s Then
            Set Printer = Printers(i)
            Exit For
        End If
    Next
    s = GetSetting(App.EXEName, "Printer", "MarginName")
    a = "select * from m_MarginPrinter where Nama='" & s & "'"
    query a
    If Not RS.EOF Then
        pLeftMargin = RS!Kiri
        pTopMargin = RS!Atas
...
Private Sub PrintReport()
    ...
    If mSQLAfterPrint <> "" Then ExecMe mSQLAfterPrint
    Printer.Orientation = mLandscape
...
On Error Resume Next
    ...
    mHide = ""
    If InStr(1, mParams, "HideCompany") <> 0 Then
        mHide = "company"
    End If
    If InStr(1, mHide, "head") = 0 Then
        For i = 0 To f.dHead.Count - 1
            PaintHeader f.dHead(i), obj, f.dHead(i), tPlus
        Next
    End If
    If InStr(1, mHide, "company") = 0 Then
        For i = 0 To f.dCompany.Count - 1
            PaintHeader f.dCompany(i), obj, f.dCompany(i), tPlus
        Next
    End If
    If InStr(1, mHide, "line") = 0 Then
    ...
    Dim tMax As Byte
    If InStr(1, mHide, "foot") = 0 Then
        For i = 0 To f.dFoot.Count - 1
            PaintDetail f.dFoot(i), obj, f.dFoot(i), Picture1.Height - f.dFooterLine.Tag - f.dFooterLine.y1 + f.dFoot(i).Top, tMax, False
        Next
        For i = 0 To f.FootLines.Count - 1
            PaintLine obj, f.FootLines(i).x1, Picture1.Height - f.dFooterLine.Tag - f.dFooterLine.y1 + f.FootLines(i).y1, f.FootLines(i).x2, Picture1.Height - f.dFooterLine.Tag - f.dFooterLine.y1 + f.FootLines(i).y2
        Next
    End If
End Sub
...
Dim iLast As String
    ...
    PrintHead obj, tPlus
    If mPageSign(mPage) = "" Then mPageSign(mPage) = "0"
    ...
    If IsNumeric(mPageSign(mPage + 1)) Then
        iLast = mPageSign(mPage + 1) - 1
    Else
        If mPage <> mTotalPage - 1 Or mTotalPage = 0 Then
            iLast = mPageSign(mPage + 1)
        Else
            iLast = "0"
        End If
